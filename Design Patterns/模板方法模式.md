...
模板方法模式建议将算法分解为一系列步骤，然后将这些步骤改写为方法，最后在公共逻辑中依次调用"模板方法"中的这些方法。
...
模板方法基于继承机制：它允许你通过扩展子类中的部分内容来改变部分算法。**策略模式**基于组合机制：你可以通过对相应行为提供不同的策略来改变对象的部分行为。
...
- ❌ 模板方法中的步骤越多，其维护工作就可能会越困难。只要算法发生变化，你就可能需要修改所有的类。
...
一个多端适配的插件，需要分别在 PC 端和 Web 端执行，这时就会涉及到公共逻辑和差异逻辑。
