...
重复创建对象会带来不必要的资源消耗、性能问题，甚至错误行为。如何确保某个类在系统中只有一个实例，并且提供**全局**访问点，避免资源竞争和数据不一致性问题，确保共享资源的安全访问。
...
在前端开发中，可以通过闭包、类、模块内实例导出等方式实现单例模式。

```typescript
export default new Instance();
```

Redux 和 Vuex 作为提供的全局 store，就是一个单例模式。

EventBus 作为事件总线，可以在组件之间传递数据或触发事件。

全局弹窗：一般情况下，同一时间只会存在一个全局弹窗。

### 响应式单例 — Vue

```typescript
Vue.observable(new Instance());
```
...
export const SingletonProvider = ({ children }: { children: ReactNode }) => {
  ...
  const singleton = { value, setValue };
